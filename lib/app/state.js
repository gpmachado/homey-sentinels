...
module.exports = {
  ...
  async _handleStateEvents(monitor, events) {
    ...
    for (const event of events) {
      ...
      const base = { device: monitor.deviceName, monitor: monitor.name, timestamp: new Date(event.timestamp).toISOString(), time: localTimeText(event.timestamp, this._getTimezone()) };

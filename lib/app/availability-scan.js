...
const { scanDevices, announceable } = require('../availability-scan');
...
module.exports = {
  ...
  async _runAvailabilityScan() {
    ...
    try {
      ...
      const announce = previous.baselined ? announceable(result.problems, settings) : [];
  ...
  async setAvailabilityExclusion(body) {
    if (body.hours !== undefined) {
      // A new silence limit for an app changes what the scan finds: look again shortly.
      this.store.setAppHours(body.app, body.hours);
      await this.store.save();
      this._scheduleAvailabilityScan(2000);
      return this.getAvailabilityScanSummary();
    }

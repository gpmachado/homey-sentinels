...
module.exports = {
  ...
  async _handleVoltageEvents(monitor, events) {
    ...
    for (const event of events) {
      ...
      const base = { device: monitor.deviceName, monitor: monitor.name, voltage: event.voltage, timestamp: new Date(event.timestamp).toISOString(), time: localTimeText(event.timestamp, this._getTimezone()) };

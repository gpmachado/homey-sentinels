...
//
// 60s still left one period per minute per monitor — ~9k per monitor across the 7-day granular
// window, ~27k for three voltage monitors, which serialized into a multi-MB blob rewritten on
// every debounced save and pushed the app past Homey's small heap ("JavaScript heap out of
// memory" inside StreamBase::WriteString/String::SlowFlatten, i.e. while writing settings).
// 5 minutes keeps the same min/max/sum/count bookkeeping (nothing here is smoothed) at ~1/5 the
// periods.
const VOLTAGE_BUCKET_MS = 5 * 60 * 1000;
...
module.exports = { VOLTAGE_BUCKET_MS, NORMAL, UNDERVOLTAGE, OVERVOLTAGE, VoltageEngine, stateFor, stabilizationGraceSeconds };

...
class SentinelStore {
  ...
  updateGroup(group, { name, type, expectedState, conjunction, messageTemplateZero, messageTemplateOne, messageTemplateMany } = {}) {
    ...
    // Applied before the caller re-validates devices against the (possibly new) type — see
    // api.js#updateGroup, which always resends the full device list on every edit, so an
    // existing device incompatible with the new type still gets caught, not silently kept.
    if (type !== undefined) { if (!GROUP_TYPES[type]) throw new Error('Unknown group type.'); group.type = type; }
  ...
  upsertAvailabilityWatchdog({ deviceId, name, thresholdHours }) {

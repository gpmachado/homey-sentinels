...
class SentinelStore {
  ...
  // One card handles both "create if needed" and "start now" — the two-step
  // add_activity_monitor-then-start_activity flow only made sense when the user already knew
  // a monitor existed; this is for wiring a brand new device in a single native-trigger Flow.
  upsertManualMonitor({ device, capability = 'measure_power', auxiliaryCapabilities = [], name, continuityMinutes = 0, minConfirmationSeconds = 0 }) {
    if (!device?.id) throw new Error('Select a valid device.');
    const existing = Object.values(this.data.monitors).find((item) => item.deviceId === device.id && item.capability === capability);
    if (existing) {
      if (name?.trim()) existing.name = name.trim();
    ...
    const monitor = {
      ...
      threshold: null, continuityMinutes: Number(continuityMinutes) || 0, minConfirmationSeconds: Number(minConfirmationSeconds) || 0,
      state: STANDBY, activeSince: null, pendingStandbySince: null, pendingActiveSince: null, lastSample: null,
      cycles: [], periods: [], totals: { cycleCount: 0, activeSeconds: 0, standbySeconds: 0, activeEnergy: 0, standbyEnergy: 0 },
      messageTemplateStarted: '', messageTemplateFinished: ''
    };

...
class SentinelStore {
  ...
  async load() {
    ...
    for (const monitor of Object.values(this.data.stateMonitors)) {
      ...
      // Both opt-in, absent for every state monitor predating this — an enum capability's
      // active value(s) (e.g. "Running") and an auto-detected power capability to track real
      // energy from, alongside the reliable state signal instead of a threshold guess.
      monitor.auxiliaryCapabilities ||= [];
    ...
    for (const monitor of Object.values(this.data.voltageMonitors)) {
      ...
      // Loaded from before the ongoing return-to-normal grace window existed — default to the
      // same 5 minutes a freshly created monitor gets, rather than 0 (instant close, the old

...
// Same bucketing idea as VoltageEngine's own VOLTAGE_BUCKET_MS (see lib/voltage-engine.js) and
// the same reason: a fast-sampling device (measure_power reporting every few seconds while
// running) otherwise pushes one period per raw sample, which is exactly the shape that caused
// a live "Memory Warning Limit Reached" crash for voltage monitors before bucketing was added
// there — confirmed live for an activity monitor too via export_data (dozens of few-second-long
// ACTIVE periods for one continuous pump run). Coalescing same-state samples within this window
// into one period, tracking min/max power instead of a single value, cuts period volume with no
// loss to _suggestedThreshold's gap search (still sees each bucket's extremes) or to
// active_duration/energy totals (summed/prorated from startedAt/endedAt either way).
const ACTIVITY_BUCKET_MS = 60 * 1000;

// Three monitor flavors share this one engine: a 'power' monitor (default) crosses a numeric
// threshold, a 'state' monitor just mirrors a boolean capability (a door, a presence sensor)
// directly, and a 'manual' monitor never decides on its own at all — its state only ever
// changes via startNow/stopNow, driven by whatever Flow logic the caller trusts (e.g. a
// native "Power becomes greater than X" trigger). Returning the monitor's own current state
// unchanged means processSample's ACTIVE/STANDBY comparison never sees a crossing to act on,
// while power/current/energy sampling into periods continues exactly as normal. Everything
// past this point (cycles, periods, grace windows, retention, median stats) is identical
// across all three; only the raw ACTIVE/STANDBY decision differs.
function stateFor(monitor, power) {
  if (monitor.mode === 'state') return power ? ACTIVE : STANDBY;
  if (monitor.mode === 'manual') return monitor.state;
  // Not calibrated yet (see SentinelStore#_parseThreshold) — stay STANDBY so nothing
  // false-triggers off `Number(null) === 0` while raw power samples accumulate for
  // app.js's auto-calibration to find a real threshold from.
  if (monitor.threshold == null) return STANDBY;
...
class ActivityEngine {
  // `power` alone decides ACTIVE/STANDBY (via stateFor) — for a 'state' monitor this is a
  // pre-resolved boolean, not a wattage. `wattage`, when given, is what actually gets stored
  // for stats (periods[].power, cycle averagePower/maxPower) instead — lets a state monitor
  // with a separate auxiliary power capability track real watts while still deciding
  // ACTIVE/STANDBY off its own reliable state signal, not a threshold guess. Omitted (every
  // existing caller), stats fall back to `power` itself — unchanged behavior.
  processSample(monitor, { power, wattage, energy = null, current = null, timestamp = Date.now() }) {
    const rawState = stateFor(monitor, power);
    const statsPower = wattage !== undefined ? Number(wattage) : Number(power);
    ...
    if (previous) {
      ...
      if (elapsedSeconds > MAX_GAP_SECONDS) {
        ...
        this._recordPeriod(monitor, { startedAt: previous.timestamp, endedAt: timestamp, state: monitor.state, seconds: elapsedSeconds, energy: energyDelta, power: previous.power, current: previous.current, meterReset });
      }
    }
    ...
    if (monitor.state === STANDBY && rawState === ACTIVE) {
      ...
      if (confirmSeconds > 0) {
        if (!monitor.pendingActiveSince) {
          ...
          this._confirmActive(monitor, monitor.pendingActiveSince, statsPower, events);
        }
      } else {
        this._confirmActive(monitor, timestamp, statsPower, events);
      }
      ...
      if (graceSeconds > 0) {
        if (!monitor.pendingStandbySince) {
          ...
          this._finalizeStandby(monitor, monitor.pendingStandbySince, statsPower, events);
        }
        // else: still within the grace window — stay ACTIVE, no event yet.
      } else {
        this._finalizeStandby(monitor, timestamp, statsPower, events);
      }
    ...
    monitor.lastSample = { power: statsPower, energy: Number.isFinite(energy) ? Number(energy) : null, current: Number.isFinite(current) ? Number(current) : null, timestamp };
    return events;
  }
  ...
  _closePreTransitionGap(monitor, timestamp) {
    ...
    if (elapsedSeconds > 0 && elapsedSeconds <= MAX_GAP_SECONDS) {
      monitor.totals[monitor.state === ACTIVE ? 'activeSeconds' : 'standbySeconds'] += elapsedSeconds;
      this._recordPeriod(monitor, { startedAt: previous.timestamp, endedAt: timestamp, state: monitor.state, seconds: elapsedSeconds, energy: null, power: previous.power, current: previous.current, meterReset: false });
    }
    monitor.lastSample = { ...previous, timestamp };
  }
  // Coalesces into the previous period instead of pushing a new one when it's still within the
  // same bucket window AND the same state — never spanning a state transition matters just like
  // for voltage: _suggestedThreshold reads minPower/maxPower per period to find the standby/
  // active split, and a bucket straddling both would blur exactly the boundary it's looking
  // for. `state` here is always the caller's own `monitor.state` read before anything in this
  // same processSample call could have mutated it (the push happens earlier in the call than
  // any transition handling) — never a value already flipped by the very transition that just
  // started this interval, avoiding the same bug class VoltageEngine's own bucketing fix
  // originally caught. Energy sums across the bucket (accumulating null only stays null when
  // every sample in it had none — a monitor either always reports energy or never does, never
  // intermittently). power/current keep both their true min/max (a brief dip or spike inside a
  // long run still shows up for calibration) AND a sum+count for a true sample-weighted average
  // later — _finalizeStandby computes a cycle's averagePower/averageCurrent from these, same
  // idea as voltageStatistics' own sample-weighted average_voltage. power is essentially always
  // finite (the engine's own state machine requires a numeric or boolean reading to run at
  // all), so its sample count always advances; current is genuinely optional (only present with
  // an auxiliary current capability), so it gets its own count that only advances when present —
  // otherwise a bucket with a couple of current-less samples would silently under-average.
  _recordPeriod(monitor, { startedAt, endedAt, state, seconds, energy, power, current, meterReset }) {
    monitor.periods ||= [];
    const lastPeriod = monitor.periods[monitor.periods.length - 1];
    if (lastPeriod && lastPeriod.state === state && (endedAt - lastPeriod.startedAt) < ACTIVITY_BUCKET_MS) {
      lastPeriod.endedAt = endedAt;
      lastPeriod.seconds += seconds;
      lastPeriod.energy = (lastPeriod.energy === null && energy === null) ? null : (lastPeriod.energy || 0) + (energy || 0);
      if (Number.isFinite(power)) {
        lastPeriod.minPower = Number.isFinite(lastPeriod.minPower) ? Math.min(lastPeriod.minPower, power) : power;
        lastPeriod.maxPower = Number.isFinite(lastPeriod.maxPower) ? Math.max(lastPeriod.maxPower, power) : power;
        lastPeriod.powerSum += power;
        lastPeriod.sampleCount += 1;
      }
      if (Number.isFinite(current)) {
        lastPeriod.maxCurrent = Number.isFinite(lastPeriod.maxCurrent) ? Math.max(lastPeriod.maxCurrent, current) : current;
        lastPeriod.currentSum += current;
        lastPeriod.currentSampleCount += 1;
      }
      lastPeriod.current = current;
      lastPeriod.meterReset = lastPeriod.meterReset || meterReset;
    } else {
      monitor.periods.push({
        startedAt, endedAt, state, seconds, energy,
        minPower: power, maxPower: power, powerSum: Number.isFinite(power) ? power : 0, sampleCount: Number.isFinite(power) ? 1 : 0,
        maxCurrent: current, currentSum: Number.isFinite(current) ? current : 0, currentSampleCount: Number.isFinite(current) ? 1 : 0,
        current, meterReset
      });
    }
    // periods are always appended in chronological order, so the oldest is always index 0 —
    // checking just that one avoids re-filtering (allocating a whole new array from) the entire
    // list on every single sample, which is nearly always a no-op this early in the app's life
    // anyway. Confirmed live: with 13k+ periods on a fast-sampling capability, doing a full
    // filter() per sample was a real, growing CPU cost behind a recurring "CPU Warning Limit
    // Reached" crash.
    const retention = endedAt - 90 * 24 * 60 * 60 * 1000;
  ...
  _finalizeStandby(monitor, endTimestamp, power, events) {
    ...
    // powerSum/sampleCount (and the current equivalents) per period — see _recordPeriod — give
    // a true sample-weighted average across a cycle's periods regardless of how many raw
    // samples got bucketed into each one; falls back to treating a legacy single-`power`-per-
    // period entry as its own one-sample bucket.
    const totalPowerSamples = cyclePeriods.reduce((sum, p) => sum + (p.sampleCount ?? (Number.isFinite(p.power) ? 1 : 0)), 0);
    const totalPowerSum = cyclePeriods.reduce((sum, p) => sum + (p.powerSum ?? (Number.isFinite(p.power) ? p.power : 0)), 0);
    const averagePower = totalPowerSamples ? totalPowerSum / totalPowerSamples : null;
    const maxPower = maximum(cyclePeriods.map((p) => p.maxPower ?? p.power).filter(Number.isFinite));
    const totalCurrentSamples = cyclePeriods.reduce((sum, p) => sum + (p.currentSampleCount ?? (Number.isFinite(p.current) ? 1 : 0)), 0);
    const totalCurrentSum = cyclePeriods.reduce((sum, p) => sum + (p.currentSum ?? (Number.isFinite(p.current) ? p.current : 0)), 0);
    const averageCurrent = totalCurrentSamples ? totalCurrentSum / totalCurrentSamples : null;
    const maxCurrent = maximum(cyclePeriods.map((p) => p.maxCurrent ?? p.current).filter(Number.isFinite));

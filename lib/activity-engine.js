  ...
  processSample(monitor, { power, wattage, energy = null, current = null, timestamp = Date.now() }) {
    ...
    if (previous) {
      ...
      if (elapsedSeconds > MAX_GAP_SECONDS) {
        ...
        // checking just that one avoids re-filtering (allocating a whole new array from) the
        // entire list on every single sample, which is nearly always a no-op this early in the
        // app's life anyway. Confirmed live: with 13k+ periods on a fast-sampling capability,
        // doing a full filter() per sample was a real, growing CPU cost behind a recurring
        // "CPU Warning Limit Reached" crash.

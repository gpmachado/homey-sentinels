...
const DEFAULT_AVAILABILITY_SETTINGS = {
  ...
  scanAnnounceSilent: false,     // announce scanned devices that are merely silent (timeline + Flow); unavailable and low battery always are
  scanAll: true,                 // also check every device that has no watchdog of its own, with these same defaults
  scanIntervalMinutes: 60        // how often that scan reads the whole device list
};
...
function normalizeAvailabilitySettings(raw) {
  ...
  return {
    ...
    scanAnnounceSilent: source.scanAnnounceSilent === undefined ? defaults.scanAnnounceSilent : source.scanAnnounceSilent === true || source.scanAnnounceSilent === 'true',
    scanAll: source.scanAll === undefined ? defaults.scanAll : source.scanAll === true || source.scanAll === 'true',
    scanIntervalMinutes: clampNumber(source.scanIntervalMinutes, 5, 24 * 60, defaults.scanIntervalMinutes)
  };
}

...
const { startOfLocalDay, recentLocalDayStarts, localDateKey } = require('./time');
...
function periodStatistics(monitor, start, end) {
  ...
  return {
    // start <= 0 is the "all" period: old cycles are pruned (see store.js CYCLE_RETENTION_DAYS), so
    // the lifetime counter is the accurate number there, not what's left in cycles[].
    cycle_count: start <= 0 ? Math.max(cycles.length, monitor.totals?.cycleCount || 0) : cycles.length, active_duration: sum(active, 'seconds') + daily.activeSeconds, standby_duration: sum(standby, 'seconds') + daily.standbySeconds,
...
function binaryEventStatistics(counter, period = 'all', timeZone, now = Date.now()) {
  ...
  const rollingWindowDays = { week: 7, month: 30 };
  const start = rollingWindowDays[period] ? recentLocalDayStarts(new Date(now), rollingWindowDays[period] + 1, timeZone)[0] : startOfLocalDay(new Date(now), timeZone).getTime();
...
// Looks for the standby/active split in a monitor's recent power history and explains itself:
// { suggestion } on success, otherwise { reason, details } saying which check stopped it and what the
// data looked like, so "why is this monitor still calibrating?" can be answered from the log
// instead of guessed. suggestedThreshold() below is the plain answer most callers want.
function analyzeThreshold(monitor) {
  const periods = monitor.periods || [];
  // Cheap length check before the sort below — called on every sample while a monitor is still
  // calibrating (see app.js's _maybeAutoCalibrate). Each period contributes up to 2 values below
  // (min+max), so this only needs half as many periods as the real threshold to be worth sorting for.
  if (periods.length * 2 < THRESHOLD_SUGGESTION_MIN_SAMPLES) return { reason: 'too_few_periods', details: { periods: periods.length } };
  // minPower/maxPower per period (see ActivityEngine#_recordPeriod's bucketing) — falls back to the
  // older single-`power`-per-period shape for anything still stored that way. Using both extremes
  // keeps the gap search seeing genuine outliers even when several raw samples got bucketed together.
  ...
  if (values.length < THRESHOLD_SUGGESTION_MIN_SAMPLES) return { reason: 'too_few_values', details: { periods: periods.length, values: values.length } };
  // A flat 10% share excluded the real gap for a lopsided duty cycle (a freezer whose compressor
  // runs most of the time). A small capped minimum still guards against a single outlier looking
  // like a cluster, without scaling up for a device that is active far more often than idle.
  ...
  const summary = {
    periods: periods.length, values: values.length, minClusterSize, min: round(values[0]), max: round(values[values.length - 1]),
    above: (limit) => values.filter((v) => v >= limit).length
  };
  const details = { periods: summary.periods, values: summary.values, minClusterSize, min: summary.min, max: summary.max, valuesAtOrAboveThreshold: summary.above(monitor.threshold ?? 0) };
  if (bestIndex === -1 || bestGap <= 0) return { reason: 'no_gap', details };
  // The two edges of the widest gap are single readings, and a single transient (a pump's start-up
  // surge, a meter glitch) sitting between the clusters made the "low" side look like 165 W instead
  // of the ~0 W the standby cluster really is — a live pump monitor stayed "calibrating" for more than
  // a week that way. So the split is judged on the clusters themselves: the 95th percentile of the low
  // side and the 5th percentile of the high side.
  const percentile = (from, to, fraction) => values[from + Math.floor((to - from) * fraction)];
  const low = percentile(0, bestIndex - 1, 0.95);
  const high = percentile(bestIndex, values.length - 1, 0.05);
  Object.assign(details, { gapLow: round(values[bestIndex - 1]), gapHigh: round(values[bestIndex]), lowRef: round(low), highRef: round(high), gapIndex: bestIndex });
  // Reject a split that's real but not convincingly a standby/active one.
  if (high < Math.max(low, 0.1) * THRESHOLD_SUGGESTION_MIN_GAP_RATIO) return { reason: 'gap_ratio_too_small', details };
  // Geometric mean lands the suggestion proportionally between the clusters rather than at their
  // arithmetic midpoint — standby and active are often an order of magnitude apart (5 W vs 1000 W).
  const threshold = Math.sqrt(Math.max(low, 0.1) * high);
  return { suggestion: { threshold: round(threshold), low, high, sampleCount: values.length } };
}

function suggestedThreshold(monitor) {
  return analyzeThreshold(monitor).suggestion || null;
}
...
module.exports = {
  ...
  dailyBreakdown, stateDailyBreakdown, binaryDailyBreakdown, suggestedThreshold, analyzeThreshold, generateTextReport
};

...
function analyzeThreshold(monitor) {
  ...
  Object.assign(details, { gapLow: round(low), gapHigh: round(high), gapIndex: bestIndex });
  // Reject a gap that's real but not convincingly a standby/active split.
  if (high < Math.max(low, 0.1) * THRESHOLD_SUGGESTION_MIN_GAP_RATIO) return { reason: 'gap_ratio_too_small', details };
  // Geometric mean lands the suggestion proportionally inside the gap rather than at its arithmetic
  // midpoint — standby and active are often an order of magnitude apart (5 W vs 1000 W).

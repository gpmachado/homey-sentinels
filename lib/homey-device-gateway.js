...
class HomeyDeviceGateway {
  constructor(homey) { this.homey = homey; this.instances = new Map(); this.deviceRefs = new Map(); this.api = null; this.deviceCache = []; }
  ...
  //
  // Held in memory only, deliberately NOT in homey.settings: ManagerSettings.set() re-serialises
  // every key on every write, so a device list (names, capabilities, values for every device on
  // the Homey) sitting there made each history save that much more expensive, and every
  // settings.get('deviceCache') parsed a fresh multi-hundred-KB copy per call. The cost of
  // memory-only is that the list is empty until the first refresh after a restart completes.
  async refreshDeviceCache() {
    this.deviceCache = await this.listDevices();
    return this.deviceCache;
  }
  getCachedDevices() { return this.deviceCache; }

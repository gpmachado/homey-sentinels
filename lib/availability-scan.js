...
function scanDevices(devices, { settings, exclusions = {}, watchedIds = new Set(), previous = {}, now = Date.now() }) {
  ...
  const appHours = exclusions.appHours || {};
  const problems = [];
  const state = {};
  let monitored = 0;
  for (const device of devices || []) {
    ...
    const result = evaluateWatchdog(
      { thresholdHours: (device.ownerUri && appHours[device.ownerUri]) || settings.defaultThresholdHours, ignoreUnavailable: false, unavailableSince: before.u ?? null, lowBatterySince: before.b ?? null },
      device, settings, now
    );
    ...
    if (reason || result.lowBattery) {
      problems.push({
        ...
        thresholdHours: (device.ownerUri && appHours[device.ownerUri]) || settings.defaultThresholdHours,
...
// Which of the new problems get announced (timeline, Flow trigger, event log). Silence alone is a
// heuristic — a switch that only reports when pressed is "silent" all day — so it is announced only when
// asked for; a device that is unavailable or has a low battery always is. A problem that is announced only
// for its battery comes out with no `reason`.
function announceable(problems, settings) {
  return problems.filter((problem) => problem.isNew).map((problem) => {
    const reason = problem.reason === 'stale' && !settings.scanAnnounceSilent ? null : problem.reason;
    return { ...problem, reason };
  }).filter((problem) => problem.reason || problem.lowBattery);
}

module.exports = { scanDevices, announceable };
